...
variable = dde.callbacks.VariableValue(
    [radius, depth], period=500, filename="variables.dat"
)

loss_weights = [1.0, 1.0, 1.0]
optimizer = torch.optim.Adam([
    {"params": model.net.parameters(), "lr": 0.0001},  # Standard LR for the network
    {"params": [radius, depth], "lr": 0.002}  # Higher LR for trainable parameters
])
...
# ========== STEP 12: Save & Plot Results ==========
dde.saveplot(losshistory, train_state, issave=True, isplot=True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"PyTorch is using: {'GPU' if torch.cuda.is_available() else 'CPU'}")

print(f"Final optimized radius: {radius.item() * L*1.43}")
print(f"Final optimized depth: {depth.item() * L/1.48}")
