...
net = dde.nn.FNN([3] + [256] * 4 + [1], "tanh", "Glorot uniform")
...
loss_weights = [1, 1, 1]
model.compile(
    "adam", lr=0.01, loss_weights=loss_weights, external_trainable_variables=external_trainable_variables
)
losshistory, train_state = model.train(iterations=5000, callbacks=[variable], display_every=500)

# ========== STEP 11: Save & Plot Results ==========
dde.saveplot(losshistory, train_state, issave=True, isplot=True)

model.save("realbreast.ckpt")
# ========== STEP 12: Verify PyTorch is Training ==========
# Check device (CPU or GPU)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"PyTorch is using: {'GPU' if torch.cuda.is_available() else 'CPU'}")

# Print updated variables
print(f"Optimized nondimensional radius: {radius.item() * L}")
print(f"Optimized nondimensional depth: {depth.item() * L}")

# Plot Training Loss
iterations = range(len(losshistory.loss_train))
plt.figure(figsize=(8,5))
plt.plot(iterations, losshistory.loss_train, label="Training Loss", color="blue")
